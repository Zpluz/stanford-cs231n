...
def gram_matrix(features, normalize=True):
    ...
    for i in range(N):
        gram_i = torch.matmul(features[i], features[i].T)
        if normalize:
            gram_i /= H * W * C
        gram[i] = gram_i
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx in range(len(style_layers)):
        cur_gram = gram_matrix(feats[style_layers[idx]])
        src_gram = style_targets[idx]
        loss += style_weights[idx] * torch.sum((cur_gram - src_gram) ** 2)
