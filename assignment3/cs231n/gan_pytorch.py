...
def sample_noise(batch_size, dim, seed=None):
    ...
    noise = torch.rand((batch_size, dim)).uniform_(-1, 1)
...
def generator_loss(logits_fake):
    ...
    fake_labels = torch.ones(size).type(dtype)
...
def get_optimizer(model):
    ...
    beta1 = 0.5
    beta2 = 0.999
    optimizer = optim.Adam(params=model.parameters(), lr=learning_rate, betas=(beta1, beta2))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    size = scores_real.shape
    true_labels = torch.ones(size).type(dtype)

    true_loss = (scores_real - true_labels) ** 2
    fake_loss = scores_fake ** 2

    loss = 0.5 * (true_loss.mean() + fake_loss.mean())
...
def ls_generator_loss(scores_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    size = scores_fake.shape
    fake_labels = torch.ones(size).type(dtype)
    fake_loss = (scores_fake - fake_labels) ** 2
